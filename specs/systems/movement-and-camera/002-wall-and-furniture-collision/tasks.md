...
- [x] T001 Add `playerRadius` (`float`, provisional default `0.5`, matching the current prototype
  scene and pending on-device re-confirmation) to the existing `GameConfig`
...
- [x] T002 Define the `Obstacle` value type (axis-aligned bounds: `min`/`max` as `Vector2`, or
  center + half-extents — pick one consistent representation) in
  `Assets/Scripts/Systems/Collision/Obstacle.cs`.
- [x] T003 Define the `CollisionResult` value type (corrected position `Vector2`) in
  `Assets/Scripts/Systems/Collision/CollisionResult.cs`.
- [x] T004 Define the `CollisionResolver` plain C# class with a static/stateless method
...
- [x] T005 Add a defensive guard in `CollisionResolver.ResolveSingle` for degenerate obstacle
...
- [x] T006 [P] [US1] EditMode test: for an oblique overlap (approach neither head-on nor parallel
...
- [x] T007 [P] [US1] EditMode test: for a position and obstacle with no overlap at all (including
...
- [x] T008 [US1] Implement the minimum-penetration-axis push-out formula in
...
- [x] T009 [P] [US2] EditMode test: for a purely perpendicular (head-on) overlap, `ResolveSingle`
...
- [x] T010 [P] [US2] EditMode test: calling `ResolveSingle` repeatedly with the same head-on input
  produces an identical result every time (no jitter/drift), in the same test file as T009.

### Implementation for User Story 2

- [x] T011 [US2] Verify/extend the T008 formula so the head-on case (penetration purely along one
...
- [x] T012 [P] [US3] EditMode test: for a position overlapping two perpendicular obstacles (a
...
- [x] T013 [P] [US3] EditMode test: resolving the same corner scenario a second time from the
  converged position returns that same position unchanged (no oscillation), in the same test file
  as T012.

### Implementation for User Story 3

- [x] T014 [US3] Implement `CollisionResolver.ResolveMany(Vector2 playerPosition, float
...
- [x] T015 [P] [US4] EditMode test: for a position starting fully inside an obstacle's bounds,
...
- [x] T016 [US4] Verify/extend T008's minimum-push-out formula correctly handles the
...
- [x] T017 [P] EditMode test: a degenerate obstacle (zero width/height or malformed bounds) never
...
- [x] T018 [P] EditMode test: a player wedged between two obstacles whose push-out vectors directly
...
- [x] T019 Implement `CharacterCollisionController` `MonoBehaviour` that, each frame after
